...
function wiki(author, category, interaction) {
    const url = `https://www.chessprogramming.org/api.php?action=query&format=json&prop=pageprops&list=categorymembers&cmtitle=Category:${category}&cmlimit=50`;
...
function formatMembers(members) {
    ...
    const url = `http://www.chessprogramming.org/api.php?action=query&format=json&prop=revisions&pageids=${pageids}&formatversion=2&rvprop=content`;
    return fetch(url, { headers: { Accept: 'application/json' } })
...
function formatRevision(page, title) {
    var embed = new EmbedBuilder()
        .setTitle(title)
        .setURL(`http://www.chessprogramming.org/index.php/${title.replace(' ','_')}`)
