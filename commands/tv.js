...
function formatChannel(channel, tv) {
    ...
    const embed = new Discord.MessageEmbed()
        ...
        .setURL(`https://lishogi.org/tv/${camel(channel)}`)
        .setDescription(`Sit back, relax, and watch the best ${channel} players compete on Lishogi TV`);
    return setGames(embed, channel);
}

function setGames(embed, channel) {
    const url = `https://lishogi.org/api/tv/${camel(channel)}?nb=5&moves=false&tags=false&opening=true`;
