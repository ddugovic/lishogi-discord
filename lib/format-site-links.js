function formatTitledUserLink(title, username) {
    ...
    return `[${username}](https://lixiangqi.com/@/${username})`;
}
...
function getTeamLinks(text) {
    const pattern = /(?<!\S)lixiangqi\.org\/team\/([-\w]+)/g;
    return text.matchAll(pattern);
}
...
function formatUserLink(text) {
    ...
    if ((match = text.match(username)))
        return `[${match[0]}](https://lixiangqi.com/@/${match[1]})`;

    const url = /^https:\/\/(?:lichess\.org|lidraughts\.org|lixiangqi\.org|playstrategy\.org)\/@\/([-\w]+)\/?$/;
    if ((match = text.match(url)))
        return `[@${match[1]}](${match[0]})`;

    const markdown = /^\[([-\w]+)\]\((https:\/\/(?:lichess\.org|lidraughts\.org|lixiangqi\.org|playstrategy\.org)\/@\/\1\/?)\)$/;
...
function formatUserLinks(text) {
    ...
    for (match of text.matchAll(username))
        text = text.replace(match[0], `[${match[0]}](https://lixiangqi.com/@/${match[1]})`);

    const url = /(?<!\S)https:\/\/(?:lichess\.org|lidraughts\.org|lixiangqi\.org|playstrategy\.org)\/@\/([-\w]+)\/?/g;
    for (match of text.matchAll(url))
        text = text.replace(match[0], `[@${match[1]}](${match[0]})`);

    const markdown = /(?<!\S)\[([-\w]+)\]\((https:\/\/(?:lichess\.org|lidraughts\.org|lixiangqi\.org|playstrategy\.org)\/@\/\1\/?)\)/g;
...
function getURIs(text) {
    const pattern = /(?<!\S)[Ll]ixiangqi\.org(?:\/[-\w]+)+/g;
    return text.matchAll(pattern);
}
