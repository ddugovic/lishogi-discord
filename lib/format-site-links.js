...
function getCoachLinks(text) {
    const pattern = /(?<!\S)lishogi\.org\/coach\/([-\w]+)/g;
    return text.matchAll(pattern);
}

>>>>>>> d1c3c79 (Extract profile bio site links (coach etc.))
...
function getTeamLinks(text) {
    const pattern = /(?<!\S)lishogi\.org\/team\/([-\w]+)/g;
    return text.matchAll(pattern);
}
...
<<<<<<< HEAD
module.exports = { formatTitledUserLink, formatArenaLink, formatArenaLinks, formatSiteLink, formatSiteLinks, formatTeamLink, formatTeamLinks, formatUserLink, formatUserLinks, formatURI, formatURIs };
=======
