...
function formatSiteLink(text) {
    return formatArenaLink(text) ?? formatCoachLink(text) ?? formatTeamLink(text) ?? formatUserLink(text) ?? formatURI(text) ?? text;
}
...
function formatArenaLink(text) {
    const arena = /(?<!\S)lichess\.org\/tournament\/([-\w]+)\/?$/;
    if ((match = text.match(arena)))
        return text.replace(match[0], `:trophy: [#${match[1]}](https://${match[0]})`);
}

function formatArenaLinks(text) {
    const arena = /(?<!\S)lichess\.org\/tournament\/([-\w]+)\/?/g;
    for (match of text.matchAll(arena))
        text = text.replace(match[0], `:trophy: [#${match[1]}](https://${match[0]})`);
    return text;
}
...
function formatCoachLink(text) {
    const coach = /(?<!\S)lichess\.org\/coach\/([-\w]+)\/?$/;
    if ((match = text.match(coach)))
        return text.replace(match[0], `[Coach](https://${match[0]})`);
}

function formatCoachLinks(text) {
    const coach = /(?<!\S)lichess\.org\/coach\/([-\w]+)\/?/g;
    for (match of text.matchAll(coach))
        text = text.replace(match[0], `[Coach](https://${match[0]})`);
    return text;
}
...
function formatTeamLink(text) {
    const battle = /(?<!\S)lichess\.org\/team\/([-\w]+)\/tournaments\/?$/;
    if ((match = text.match(battle)))
        return `[${title(match[1])} Battles](https://${match[0]})`;
    const team = /(?<!\S)lichess\.org\/team\/([-\w]+)\/?$/;
    if ((match = text.match(team)))
        return `[${title(match[1])}](https://${match[0]})`;
}

function formatTeamLinks(text) {
    const battle = /(?<!\S)lichess\.org\/team\/([-\w]+)\/tournaments\/?/g;
    for (match of text.matchAll(battle))
        text = text.replace(match[0], `[${title(match[1])} Battles](https://${match[0]})`);
    const team = /(?<!\S)lichess\.org\/team\/([-\w]+)\/?/g;
    for (match of text.matchAll(team))
        text = text.replace(match[0], `[${title(match[1])}](https://${match[0]})`);
    return text;
}
...
function formatUserLink(text) {
    ...
    const url = /(?<!\S)(?:lichess\.org|lidraughts\.org|lishogi\.org|playstrategy\.org)\/@\/([-\w]+)\/?$/;
    if ((match = text.match(url)))
        return `[@${match[1]}](https://${match[0]})`;
...
function formatUserLinks(text) {
    ...
    const url = /(?<!\S)(?:lichess\.org|lidraughts\.org|lishogi\.org|playstrategy\.org)\/@\/([-\w]+)\/?/g;
    for (match of text.matchAll(url))
        text = text.replace(match[0], `[@${match[1]}](https://${match[0]})`);
...
function formatGifURL(gameId, theme, piece) {
    if (theme && piece) {
        return `https://lichess1.org/game/export/gif/${gameId}.gif?theme=${theme}&piece=${piece}&glyphs=true&clocks=true`;
    }
    return `https://lichess1.org/game/export/gif/${gameId}.gif?glyphs=true&clocks=true`;
}
...
function formatThumbnailURL(gameId, theme, piece) {
    if (theme && piece) {
        return `https://lichess1.org/game/export/gif/thumbnail/${gameId}.gif?theme=${theme}&piece=${piece}`;
    }
    return `https://lichess1.org/game/export/gif/thumbnail/${gameId}.gif`;
}
...
module.exports = { formatTitledUserLink, formatArenaLink, formatArenaLinks, formatGifURL, formatPositionURL, formatSiteLink, formatSiteLinks, formatTeamLink, formatTeamLinks, formatThumbnailURL, formatUserLink, formatUserLinks, formatURI, formatURIs, getSiteLinks };
