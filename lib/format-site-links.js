...
function formatSiteLink(text) {
    return formatArenaLink(text) ?? formatTeamLink(text) ?? formatUserLink(text) ?? text;
}

function formatSiteLinks(text) {
    return formatArenaLinks(formatTeamLinks(formatUserLinks(text)));
}

function formatArenaLink(text) {
    const arena = /^https:\/\/lichess.org\/tournament\/[-\w]+\/?$/;
    if ((match = text.match(arena)))
        return `[Arena](${match[0]})`;
}

function formatArenaLinks(text) {
    const arena = /^https:\/\/lichess.org\/tournament\/([-\w]+)\/?/g;
...
module.exports = { formatTitledUserLink, formatArenaLink, formatArenaLinks, formatSiteLink, formatSiteLinks, formatTeamLink, formatTeamLinks, formatUserLink, formatUserLinks };
