...
function decodeWord(lexicon, word) {
    if ('FISE2'.equals(lexicon)) {
        return word.replaceAll('1','[CH]').replaceAll('2','[LL]').replaceAll('3','[RR]');
    }
    return word;
}
...
function encodeWord(lexicon, word) {
    if ('FISE2'.equals(lexicon)) {
        return word.replaceAll('[CH]','1').replaceAll('[LL]','2').replaceAll('[RR]','3');
    }
    return word;
}
...
module.exports = { decodeWord, encodeWord, formatLexicon, getLexiconCategory };
