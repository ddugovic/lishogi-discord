...
function formatLink(text) {
    if (text.match(/\s+/)) {
        if (urlRegexSafe().test(text)) {
            const words = text.trim().split(/[-:]?\s+/);
            if (urlRegexSafe({exact: true}).test(words[0]))
                return `[${words.slice(1).map(title).join(' ')}](${words[0]})`;
            if (urlRegexSafe({exact: true}).test(words[words.length - 1]))
                return `[${words.slice(0, -1).map(title).join(' ')}](${words[words.length - 1]})`;
        }
    } else if (urlRegexSafe({exact: true, strict: false}).test(text)) {
        if (text.match(/^https:\/\//))
            return `[${text}](${text})`;
...
function formatSocialLinks(text) {
    ...
    for (user of getImgur(text))
        links.push(`[Imgur](https://${user})`);
...
function getImgur(text) {
    const pattern = /imgur\.com\/\w+/g;
    return text.matchAll(pattern);
}
