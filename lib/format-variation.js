...
async function formatSanVariation(fen, sans) {
    const { makeSanVariation, parseSan } = await import('chessops/san');
...
async function setup(fen) {
    const { INITIAL_FEN, parseFen } = await import('chessops/fen');
    const { Chess } = await import('chessops/chess');
    const setup = parseFen(fen ?? INITIAL_FEN).unwrap();
    return Chess.fromSetup(setup).unwrap();
}

module.exports = { formatOpening, formatSanVariation, formatUciVariation, numberVariation };
